...
def run_specific_workflow_tests():
    ...
    if choice == "all":
        return run_tests(argparse.Namespace(verbose=True, test=None, headless=False, coverage=False, serial=True))
    elif choice in workflows:
        return run_tests(
            argparse.Namespace(verbose=True, test=workflows[choice], headless=False, coverage=False, serial=True)
        )
    else:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Run SpritePal UI workflow integration tests")

    parser.add_argument(
        "--headless", action="store_true", help="Run tests in headless mode (uses Qt offscreen platform)"
    )

    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output (don't capture stdout)")

    parser.add_argument("--test", "-k", help="Run specific test (pytest -k pattern)")

    parser.add_argument("--coverage", action="store_true", help="Generate coverage report")

    parser.add_argument("--serial", action="store_true", help="Force serial execution (no parallel tests)")

    parser.add_argument("--interactive", "-i", action="store_true", help="Interactive mode - choose specific workflows")
