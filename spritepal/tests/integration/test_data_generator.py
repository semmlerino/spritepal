...
class TestROMGenerator:
    ...
    @staticmethod
    def insert_sprite_at_offset(
        rom_data: bytearray, offset: int, sprite_data: bytes, compress: bool = False
    ) -> dict[str, Any]:
        """
    ...
    @staticmethod
    def create_rom_with_sprites(
        sprites: list[dict[str, Any]], rom_size: int = 4 * 1024 * 1024
        ...
        for sprite_def in sprites:
            ...
            # Insert into ROM
            sprite_info = TestROMGenerator.insert_sprite_at_offset(rom_data, offset, sprite_data, compress)
    ...
    def save_test_rom(path: Path, rom_data: bytes, sprite_infos: list[dict[str, Any]] | None = None):
        ...
        if sprite_infos:
            ...
            metadata = {"rom_file": path.name, "size": len(rom_data), "sprites": sprite_infos}
