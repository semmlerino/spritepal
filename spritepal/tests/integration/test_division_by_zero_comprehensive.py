...
class TestDivisionByZeroFixes:
    ...
    def test_preview_worker_zero_expected_size(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".rom") as tmp:
            ...
            worker = SpritePreviewWorker(
                rom_path=tmp.name, offset=0, sprite_name="test_sprite", extractor=MagicMock(), sprite_config=None
            )
    ...
    def test_search_worker_zero_step(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".rom") as tmp:
            ...
            worker = SpriteSearchWorker(
                rom_path=tmp.name, start_offset=0, end_offset=100, direction=1, extractor=MagicMock()
            )
    ...
    def test_all_workers_with_realistic_edge_cases(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".rom") as tmp:
            ...
            assert scan_worker._parallel_finder.search_parallel.called, "Scan worker should complete search"
        ...
        for start, end in test_cases:
            with tempfile.NamedTemporaryFile(suffix=".rom") as tmp:
                ...
                try:
                    ...
                    pytest.fail(f"ZeroDivisionError with start={start}, end={end}")
