...
-- ============================================================================
-- Step A: OBSEL Latch - $2101 is a write-only PPU port on real hardware
-- Reading it directly is unreliable. We latch writes via memory callback.
-- ============================================================================
local LAST_OBSEL = nil

local function install_obsel_latch()
    if not emu.addMemoryCallback then
        log("WARN: addMemoryCallback not available; OBSEL will be read (less reliable).")
        return
    end

    local cbWrite =
        (emu.callbackType and emu.callbackType.write) or
        (emu.memCallbackType and emu.memCallbackType.write)

    if not cbWrite then
        log("WARN: Could not find callbackType.write; OBSEL latch not installed.")
        return
    end

    emu.addMemoryCallback(function(address, value)
        LAST_OBSEL = value & 0xFF
        log(string.format("OBSEL written: 0x%02X", LAST_OBSEL))
    end, cbWrite, 0x2101, 0x2101, emu.cpuType.snes, emu.memType.snesMemory)

    log("OBSEL latch installed (captures writes to $2101).")
end
...
-- Uses bit replication: RRRRR → RRRRR_RRR (shift left by 3)
...
-- Get OBSEL settings (uses latched value if available, else reads port)
local function get_obsel()
    local obsel
    if LAST_OBSEL ~= nil then
        obsel = LAST_OBSEL
        log(string.format("Using latched OBSEL: 0x%02X", obsel))
    else
        obsel = emu.read(0x2101, emu.memType.snesMemory) or 0
        log(string.format("WARN: Reading OBSEL from port (unreliable): 0x%02X", obsel))
    end
...
local function parse_oam_entry(index)
    ...
    -- Main table (4 bytes per entry) - in SnesSpriteRam
    local x_low = emu.read(base + 0, emu.memType.snesSpriteRam)
    ...
    -- High table (2 bits per entry at offset 0x0100)
    local hi_byte_idx = 0x0100 + math.floor(index / 4)
    local hi_byte = emu.read(hi_byte_idx, emu.memType.snesSpriteRam)
...
-- Step B: Use emu.read16 for deterministic VRAM reads (instead of readWord)
local function read_vram_word(byte_addr)
    -- Prefer emu.read16 (more reliable in Mesen 2)
    if emu.read16 then
        local word = emu.read16(byte_addr, MEM.vram)
        if word then
            return word
        end
    end
    -- Try emu.readWord as second choice
    ...
    -- Fallback: read two consecutive bytes and combine them (little-endian)
...
local function read_vram_tile_word(vram_addr)
    ...
    for i = 0, 15 do
        ...
        -- read_vram_word returns little-endian (low byte at low address)
        -- SNES tile bytes should be stored in sequential order: byte0, byte1, ...
        -- So low byte of word goes first, then high byte
        tile_data[i * 2 + 1] = word & 0xFF          -- low byte first
        tile_data[i * 2 + 2] = (word >> 8) & 0xFF   -- high byte second
...
-- ============================================================================
-- Step C: Memory dump helper - dump VRAM/CGRAM/OAM to file for comparison
-- ============================================================================
local function dump_mem_to_file(path, memType, label)
    local size = emu.getMemorySize(memType)
    if not size or size == 0 then
        log("ERROR: Cannot get memory size for " .. (label or "unknown"))
        return false
    end

    local f = io.open(path, "wb")
    if not f then
        log("ERROR: couldn't write dump: " .. path)
        return false
    end
    ...
    local addr = 0
    while addr < size do
        local n = math.min(CHUNK, size - addr)
        local t = {}
        for i = 0, n - 1 do
            t[#t+1] = string.char(emu.read(addr + i, memType) or 0)
        end
    ...
    f:close()
    log(string.format("Wrote %s dump %s (%d bytes)", label or "memory", path, size))
    return true
end

-- Dump all memory for debugging (VRAM, CGRAM, OAM)
local function dump_all_memory()
    local timestamp = os.time()
    local base_path = OUTPUT_DIR .. "lua_dump_" .. timestamp .. "_"

    dump_mem_to_file(base_path .. "VRAM.dmp", MEM.vram, "VRAM")
    dump_mem_to_file(base_path .. "CGRAM.dmp", MEM.cgram, "CGRAM")
    dump_mem_to_file(base_path .. "OAM.dmp", MEM.oam, "OAM")

    log("Memory dumps complete: " .. base_path .. "*.dmp")
    print("SpritePal: Memory dumps saved to " .. base_path .. "*.dmp")
...
local function capture_sprites()
    log("Capture triggered at frame " .. frameCount)

    -- Dump memory for Step C comparison
    local dump_path = dump_all_memory()
    ...
    -- DEBUG MODE: Capture ALL entries regardless of visibility
    -- This helps identify whether sprites are in OAM during the captured frame
    for i = 0, 127 do
        local entry = parse_oam_entry(i)

        -- Track visible for statistics
        ...
        -- SNES OBJ tiles: base tile is split into X (low nibble) and Y (high nibble)
        -- OBJ stepping does NOT carry: X wraps within low nibble, Y within high nibble
        -- Example: 0x1F + right = 0x10 (not 0x20)
        local base_tile = entry.tile
        local base_x = base_tile & 0x0F
        local base_y = (base_tile >> 4) & 0x0F
        local use_second_table = entry.name_table == 1

        for ty = 0, tiles_y - 1 do
            local y = (base_y + ty) & 0x0F
            for tx = 0, tiles_x - 1 do
                local x = (base_x + tx) & 0x0F
                local tile_index = (y << 4) | x
        ...
        -- Add ALL entries to output (not just visible ones)
    ...
    log("Captured " .. visible_count .. " visible + " .. (#entries - visible_count) .. " off-screen sprites (total " .. #entries .. ") to " .. filename)
    print("SpritePal: Captured " .. visible_count .. " visible + " .. (#entries - visible_count) .. " off-screen sprites (total " .. #entries .. ") to " .. filename)
end

-- Track previous state for edge detection
local lastCaptureState = false
local lastKeyState = false
local lastDumpKeyState = false
...
emu.addEventCallback(function()
    ...
    emu.drawString(8, 8, "SpritePal: C=Capture, D=Dump (or Start+Select)", 0xFFFFFF, 0x80000000)
    emu.drawString(8, 20, "Frame: " .. frameCount .. (LAST_OBSEL and string.format(" OBSEL:0x%02X", LAST_OBSEL) or " OBSEL:??"), 0xFFFFFF, 0x80000000)

    -- Check keyboard C key (less likely to be reserved than F9)
    local keyPressed = emu.isKeyPressed("C")
    local dumpKeyPressed = emu.isKeyPressed("D")

    -- Check controller input: Start+Select together
    local input = emu.getInput(0) or {}
    local controllerPressed = input.start and input.select

    -- Debug: show key state on screen
    if keyPressed then
        emu.drawString(8, 32, "C KEY DETECTED!", 0x00FF00, 0x80000000)
    end
    if dumpKeyPressed then
        emu.drawString(8, 32, "D KEY DETECTED - DUMPING!", 0xFFFF00, 0x80000000)
    end

    -- Edge detection for D key - just dump memory
    if dumpKeyPressed and not lastDumpKeyState then
        log("D key pressed at frame " .. frameCount)
        print("SpritePal: D key pressed - dumping memory only!")
        emu.drawString(8, 44, "DUMPING MEMORY...", 0xFFFF00, 0x80000000)
        dump_all_memory()
    end
    lastDumpKeyState = dumpKeyPressed

    -- Edge detection for keyboard (only trigger once per press)
    if keyPressed and not lastKeyState then
        log("C key pressed at frame " .. frameCount)
        print("SpritePal: C key pressed - capturing!")
        emu.drawString(8, 44, "CAPTURING...", 0x00FF00, 0x80000000)
        capture_sprites()
    end
    lastKeyState = keyPressed

    -- Edge detection for controller (only trigger once per press)
    if controllerPressed and not lastCaptureState then
        log("Start+Select detected at frame " .. frameCount)
        print("SpritePal: Start+Select pressed - capturing!")
        emu.drawString(8, 44, "CAPTURING...", 0x00FF00, 0x80000000)
        capture_sprites()
    end
    lastCaptureState = controllerPressed
end, emu.eventType.endFrame)

-- Startup message
log("Script running, press C or Start+Select to capture sprites, D to dump memory")
print("==============================================")
print("SpritePal Sprite Capture Script LOADED (with Step A/B/C patches)")
print("  C = Full capture (sprites + memory dumps)")
print("  D = Memory dump only (for Step C comparison)")
print("  Start+Select (controller) = Full capture")
print("Look for on-screen text to confirm script is active")
print("==============================================")
