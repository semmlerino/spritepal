...
def reconstruct_frame(
    ...
    Draws sprites in priority order (low priority first, high on top).
    Handles wrap-around for sprites at screen edges.
    """
    # Create screen-sized canvas with checkerboard background for visibility
    canvas = Image.new("RGBA", (SNES_WIDTH, SNES_HEIGHT), (48, 48, 48, 255))

    # Sort by priority (lower priority = drawn first, appears behind)
    # Then by OAM ID (higher ID = drawn later in SNES hardware, appears on top)
    sorted_sprites = sorted(sprites, key=lambda s: (s.priority, s.id))

    for sprite in sorted_sprites:
        # Skip sprites in overscan zone (y=224-239 means "off-screen")
        if 224 <= sprite.y < 240:
            continue

        sprite_img = render_sprite(sprite, palettes)

        # Calculate screen position (handle negative X and wrap)
        x = sprite.x
        y = sprite.y

        # SNES Y wraps at 256 but visible area is 0-223
        # X can be -256 to +255 (9-bit signed)

        # Paste sprite at position
        # For sprites partially off-screen, we just paste what fits
        ...
        if show_bounds:
            ...
            draw.rectangle([x, y, x + sprite.width - 1, y + sprite.height - 1], outline=(255, 0, 255, 128))
...
def cluster_sprites(
    ...
    visible = [s for s in sprites if not (224 <= s.y < 240)]
    if not visible:
        return []
