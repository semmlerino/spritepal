...
def parse_fill_sessions(log_path: Path) -> list:
    ...
    with open(log_path, 'r') as f:
        for line in f:
            ...
            match = re.search(r'FILL_BUFFER_BYTES: frame=(\d+) addr_range=0x([0-9A-Fa-f]+)-0x([0-9A-Fa-f]+) bytes=(\d+) hex=([0-9A-Fa-f-]+)', line)
...
def compare_session(session: dict, rom_data: bytes) -> dict:
    ...
    for run in sorted_runs:
        ...
        if pos >= 0:
            ...
            for start in range(len(buffer_bytes) - 8):
                for rom_start in range(len(rom_bytes) - 8):
                    match_len = 0
                    while (start + match_len < len(buffer_bytes) and
                           rom_start + match_len < len(rom_bytes) and
                           buffer_bytes[start + match_len] == rom_bytes[rom_start + match_len]):
                        match_len += 1
        ...
        if match_info['exact_match'] or (match_info['match_length'] > 0 and
            (result['best_match'] is None or match_info['match_length'] > result['best_match']['match_length'])):
