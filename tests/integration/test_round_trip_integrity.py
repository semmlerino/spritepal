...
class TestEditingControllerROMValidation:
    """Test EditingController ROM constraint validation."""
    ...
    def test_valid_dimensions_pass_validation(self, editing_controller):
        """Verify 8-pixel aligned dimensions pass validation."""
        # Load a valid 16x16 sprite (divisible by 8)
        data = np.zeros((16, 16), dtype=np.uint8)
        editing_controller.load_image(data, [])

        assert editing_controller.is_valid_for_rom() is True
        assert len(editing_controller.get_validation_errors()) == 0

    def test_invalid_dimensions_fail_validation(self, editing_controller):
        """Verify non-8-pixel aligned dimensions fail validation."""
        # Load a 17x16 sprite (not divisible by 8)
        data = np.zeros((16, 17), dtype=np.uint8)
        editing_controller.load_image(data, [])

        assert editing_controller.is_valid_for_rom() is False
        errors = editing_controller.get_validation_errors()
        assert len(errors) > 0
        assert any("multiple of 8" in err.lower() for err in errors)

    def test_too_many_colors_fail_validation(self, editing_controller):
        """Verify more than 16 colors fail validation."""
        # Create data with 17 unique color indices
        data = np.arange(64, dtype=np.uint8).reshape(8, 8)
        # First 64 pixels will have values 0-63, way more than 16 unique
        editing_controller.load_image(data, [])

        assert editing_controller.is_valid_for_rom() is False
        errors = editing_controller.get_validation_errors()
        assert len(errors) > 0
        assert any("colors" in err.lower() or "16" in err for err in errors)

    def test_exactly_16_colors_passes_validation(self, editing_controller):
        """Verify exactly 16 colors passes validation."""
        # Create 8x8 data using all 16 colors (0-15)
    ...
    def test_validation_signal_emitted_on_load_invalid(self, editing_controller, qtbot):
        """Verify validationChanged signal is emitted when loading invalid image."""
        # Start with valid image to establish baseline
        valid_data = np.zeros((8, 8), dtype=np.uint8)
        editing_controller.load_image(valid_data, [])

        # Now load invalid image - should trigger signal because state changes
        invalid_data = np.zeros((17, 8), dtype=np.uint8)  # Non-aligned width

        with qtbot.waitSignal(editing_controller.validationChanged, timeout=1000) as blocker:
            editing_controller.load_image(invalid_data, [])

        # Verify signal args: (is_valid=False, errors=["..."])
        assert blocker.args[0] is False

    def test_validation_signal_emitted_on_state_change(self, editing_controller, qtbot):
        """Verify validationChanged signal emitted when validation state changes."""
        # Load invalid image first
        invalid_data = np.zeros((17, 8), dtype=np.uint8)
        editing_controller.load_image(invalid_data, [])

        # Now load valid image - state changes from invalid to valid
        valid_data = np.zeros((8, 8), dtype=np.uint8)

        with qtbot.waitSignal(editing_controller.validationChanged, timeout=1000) as blocker:
            editing_controller.load_image(valid_data, [])

        # Verify signal args: (is_valid=True, errors=[])
        assert blocker.args[0] is True

    def test_force_validation_triggers_signal_on_state_change(self, editing_controller, qtbot):
        """Verify force_validation() emits signal when state changes."""
        # Start with invalid image
        invalid_data = np.zeros((17, 8), dtype=np.uint8)
        editing_controller.load_image(invalid_data, [])

        # Manually update image model to valid dimensions (simulating external change)
        valid_data = np.zeros((8, 8), dtype=np.uint8)
        editing_controller.image_model.data = valid_data
        editing_controller.image_model.width = 8
        editing_controller.image_model.height = 8

        # Now force validation - should detect state change
        with qtbot.waitSignal(editing_controller.validationChanged, timeout=1000) as blocker:
