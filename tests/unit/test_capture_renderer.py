...
def test_render_selection_crops_to_bbox():
    ...
    capture = CaptureResult(
        frame=100, visible_count=2, obsel=OBSELConfig(0, 0, 0, 0, 0, 0, 0), entries=entries, palettes={0: [0] * 16}
    )
...
def test_render_selection_empty_capture():
    capture = CaptureResult(frame=100, visible_count=0, obsel=OBSELConfig(0, 0, 0, 0, 0, 0, 0), entries=[], palettes={})
    renderer = CaptureRenderer(capture)
