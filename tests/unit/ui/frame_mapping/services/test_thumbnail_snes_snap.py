...
class TestQuantizePilImageSnesSnap:
    ...
    def test_quantize_snaps_palette_to_snes_colors(self) -> None:
        ...
        palette = FakeSheetPalette(
            colors=_pad_palette([(0, 0, 0), non_snes_color])
        )
        ...
        for pixel in opaque_pixels:
            assert pixel[:3] == expected_snapped, (
                f"Expected SNES-snapped {expected_snapped}, got {pixel[:3]}"
            )
    ...
    def test_quantize_already_snes_colors_unchanged(self) -> None:
        ...
        palette = FakeSheetPalette(
            colors=_pad_palette([(0, 0, 0), snes_color])
        )
    ...
    def test_multiple_non_snes_colors_all_snapped(self) -> None:
        ...
        palette = FakeSheetPalette(
            colors=_pad_palette([(0, 0, 0)] + non_snes_colors)
        )
        ...
        for pixel in opaque_pixels:
            ...
            assert pixel[:3] == snapped_expected[0], (
                f"Expected snapped {snapped_expected[0]}, got {pixel[:3]}"
            )
