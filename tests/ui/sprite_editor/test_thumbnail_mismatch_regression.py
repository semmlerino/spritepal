...
def test_thumbnail_generation_uses_stale_data():
    ...
    assert controller.current_tile_data is None, (
        "Data should be CLEARED immediately after set_offset to prevent stale usage"
    )
    ...
    assert use_cached_data is False, (
        "Controller should NOT use stale data because data was cleared and offset invalidated"
    )
